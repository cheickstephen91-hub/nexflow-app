  ...
  return (
    <div className="min-h-screen relative" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-0 left-1/4 w-[400px] h-[400px] rounded-full blur-3xl" style={{ background: 'rgba(34,197,94,0.03)' }} />
      </div>

      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Aide &amp; Documentation</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Guides, FAQ et support</p>
        </div>

        {/* ── Guide démarrage ── */}
        <SectionCard>
          <h2 className="font-semibold text-base mb-4 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
            <span className="w-6 h-6 rounded-full flex items-center justify-center shrink-0" style={{ background: 'var(--accent-light)', color: 'var(--accent)' }}>
            ...
            {STEPS.map((s) => {
              ...
              return (
                <li key={s.n}>
                  ...
                  >
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: 'var(--accent-light)', color: 'var(--accent)', border: '1px solid var(--accent)' }}>
                      {s.n}
                    </div>
                    <p className="flex-1 text-sm font-semibold transition-colors" style={{ color: 'var(--text-primary)' }}>
                      {s.label}
                    </p>
                    <svg
                  ...
                  >
                    <p className="text-xs leading-relaxed pl-10 pb-3 pr-2" style={{ color: 'var(--text-secondary)' }}>
                      {s.desc}
                    </p>
                  </div>
                  {s.n < STEPS.length && <div className="h-px ml-10" style={{ background: 'var(--border)' }} />}
                </li>
              )
            })}
          </ol>
        ...
        {/* ── FAQ ── */}
        <SectionCard>
          <h2 className="font-semibold text-base mb-4" style={{ color: 'var(--text-primary)' }}>Questions fréquentes</h2>
          <div className="space-y-0">
            {FAQ.map((item, i) => {
              const isOpen = openFaq === i
              return (
                <div key={i} style={{ borderBottom: i < FAQ.length - 1 ? '1px solid var(--border)' : undefined }}>
                  <button
                    onClick={() => setOpenFaq(isOpen ? null : i)}
                    className="w-full flex items-start gap-2 py-4 text-left"
                  >
                    <span className="text-sm font-bold shrink-0 mt-0.5" style={{ color: 'var(--accent)' }}>Q.</span>
                    <p className="flex-1 text-sm font-semibold transition-colors" style={{ color: 'var(--text-primary)' }}>
                      {item.q}
                    </p>
                    <svg
                      className={`w-4 h-4 shrink-0 mt-0.5 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
                      style={{ color: 'var(--accent)' }}
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                    >
                      <path d="M6 9l6 6 6-6"/>
                  ...
                  >
                    <p className="text-xs leading-relaxed pl-5 pb-4" style={{ color: 'var(--text-secondary)' }}>
                      {item.r}
                    </p>
                  </div>
                </div>
        ...
        <SectionCard>
          <h2 className="font-semibold text-base mb-5" style={{ color: 'var(--text-primary)' }}>Documentation technique</h2>
          <div className="space-y-4">
            <div className="rounded-xl p-4" style={{ background: 'var(--bg-card-hover)', border: '1px solid var(--border)' }}>
              <p className="text-sm font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>Comment exporter les données ?</p>
              <p className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>Dans le tableau de bord, un bouton &ldquo;Exporter&rdquo; permet de télécharger toutes les fiches en format Excel ou CSV.</p>
            </div>
            <div className="rounded-xl p-4" style={{ background: 'var(--bg-card-hover)', border: '1px solid var(--border)' }}>
              <p className="text-sm font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>Comment signaler un bug ?</p>
              <p className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>Utilisez le bouton &ldquo;Contacter le support&rdquo; ci-dessous pour nous signaler tout problème.</p>
            </div>
          </div>
        </SectionCard>
        ...
        {/* ── Support ── */}
        <SectionCard>
          <h2 className="font-semibold text-base mb-2" style={{ color: 'var(--text-primary)' }}>Support</h2>
          <p className="text-xs mb-5" style={{ color: 'var(--text-secondary)' }}>Notre équipe répond dans les 24 heures, du lundi au vendredi.</p>
          <div className="flex flex-col sm:flex-row gap-3">
            <a
              href="https://wa.me/2250777842576"
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 flex items-center justify-center gap-2.5 font-semibold text-sm py-3.5 px-5 rounded-xl transition-all duration-200"
